...
'use strict';
angular.module('rescour.marketplace', ['rescour.config']).service('Market', [
  'Dimensions',
  function (Dimensions, $q) {
    ...
    this.render = function (subset) {
      var self = this, dimensions = this.dimensions;
      self.visibleIds = [];
      ...
      for (i = 0; i < BIT_SET_LENGTH; i++) {
        ...
        for (var attrId in dimensions.discreet) {
          if (dimensions.discreet.hasOwnProperty(attrId)) {
            var _discreet = dimensions.discreet[attrId], union = 0;
        ...
        for (var p = 0; p < 32; p++) {
          ...
          if (dimensions.idMap[itemIndex]) {
            ...
            if (!_.isEmpty(dimensions.range)) {
              for (var _rangeKey in dimensions.range) {
                if (dimensions.range.hasOwnProperty(_rangeKey)) {
                  var _rangeAttr = dimensions.range[_rangeKey], _currItemAttr = _currItem[_rangeKey];
                  if (_currItemAttr >= _rangeAttr.lowSelected && _currItemAttr <= _rangeAttr.highSelected || _currItemAttr === 'NA') {
  ...
  function ($timeout) {
    ...
    function Dimensions(dimensions) {
      ...
      for (var attrID in dimensions.discreet) {
        if (dimensions.discreet.hasOwnProperty(attrID)) {
          var _attr = dimensions.discreet[attrID], _discreet = angular.extend(_attr, discreetDefaults);
      ...
      for (var attrID in dimensions.range) {
        if (dimensions.range.hasOwnProperty(attrID)) {
          var _attr = dimensions.range[attrID], _range = angular.extend(_attr, rangeDefaults);
          self.range[attrID] = {};
    ...
    return {
      ...
      link: function (scope, element, attrs) {
        var opts = {
          ...
          }, ele = element[0], spinner = new $spinner(opts[attrs.spinnerSize || 'small']);
        scope.$watch(function () {
...
angular.module('rescour.config', []).factory('$_api', [
  '$http',
  function ($http) {
    var url = { dev: 'http://dev.maasive.net/v2/rescour' }, stripeTokens = {
  ...
  function ($_api, $q, $http, States, $rootScope) {
    function ItemFactory(collection) {
      ...
      Item.query = function (tag) {
        var defer = $q.defer(), config = angular.extend({
            transformRequest: function (data) {
              return data;
            }
          }, $_api.config), path = tag ? $_api.path + Item.path + tag : $_api.path + Item.path;
        path += '?limit=5000';
        $http.get(path, config).then(function (response) {
          console.log(path);
          defer.resolve(response);
